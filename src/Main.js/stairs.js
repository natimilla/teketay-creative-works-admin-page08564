...
function Stairs() {
  ...
  const submitHandler = async (event) => {
    ...
    try {
      const response = await fetch(
        "https://teketay-creative-works-default-rtdb.firebaseio.com/stairs.json",
        {
          method: "POST",
  ...
  const removeHandler = async(id) => {
    ...
    try {
      ...
      const itemRef = REF(database, `stairs/${id}`);
  ...
  return (
    <div className={classes.container}>
      <div>
        <div className={classes.heading}>Stair Handles</div>
